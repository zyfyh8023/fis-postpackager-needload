...
module.exports = function(ret, conf, settings, opt) { //打包后处理
     ...
     */
    function injectJs(jsList, cssList, content) {
        var cssArr = {};
        var script = jsList.reduce(function(prev, now) {
            ...
            if (cssList) {
                for (var i = 0; i < cssList.length; i++) {
                    if (cssList[i].uri) {
                        var tempVal = cssList[i].uri.match(/.*\//);
                        if (tempVal) {
                            if (uri.match(tempVal) && !cssArr[cssList[i].uri]) {
                                cssArr[cssList[i].uri] = true;
                            }
                        }
                    }
                }
        ...
        var cssLinks = '';
        for (var p in cssArr) {
            if (cssArr[p] && cssArr.hasOwnProperty(p)) {
                cssLinks += '<link rel="stylesheet" type="text/css" href="' + p + '">\r\n';
            }
        }
        if (content.indexOf(settings.scriptTag) !== -1) {
            content = content.replace(settings.scriptTag, cssLinks + script);
        } else {
            content = content.replace(/<\/head>/, cssLinks + script + '\n$&');
        }
        return content;
    }
     ...
     * @returns {XML|string|void}
     */
    function injectSiteAsync(content, cssList, jsList) {
        ...
        return injectAsyncWithMap(content, siteAsync, cssList, jsList);
    }
     ...
     * @returns {*}
     */
    function injectAsync(asyncList, content, usedSync, filePath, cssList, jsList) {
        ...
        asyncCount++;
        return injectAsyncWithMap(content, file, cssList, jsList);
    }

    function injectAsyncWithMap(content, resourceMapFile, cssList, jsList) {
        var mapScript;
        ...
        for (var k in jsList) {
            for (var i = 0; i < cssList.length; i++) {
                var tempVal = cssList[i].uri.match(/.*\//);
                if (jsList.hasOwnProperty(k) && tempVal && jsList[k].uri.match(tempVal)) {
                    cssJsSign[cssList[i].uri] = true;
                }
            }
        }
        for (var q in cssList) {
            if (cssList.hasOwnProperty(q) && cssJsSign[q]){
                cssList[q].loadSign=true;
            }
        }
        //未剔除同步的css依赖文件
        var J_cssList = JSON.stringify(cssList);
        J_cssList = 'require.cssList(' + J_cssList + ')';
        var tempCss = '<script type="text/javascript" >' + J_cssList + '\r\n</script>';

        if (content.indexOf(settings.resourceMapTag) !== -1) {
            content = content.replace(settings.resourceMapTag, tempCss + mapScript);
        } else {
            content = content.replace(/<\/head>/, tempCss + mapScript + '\n$&');
        }
        return content;
    }
    ...
    function injectAutoLoad(file, include) {
        ...
        if (settings.useSiteMap) {
            content = injectSiteAsync(content, cssList, jsList);
        } else {
            content = injectAsync(asyncList, content, usedSync, file.subpath, cssList, jsList);
        }
        content = injectJs(jsList, cssList, content);
